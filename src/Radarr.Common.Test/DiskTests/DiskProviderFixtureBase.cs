...
namespace Radarr.Common.Test.DiskTests
{
    public abstract class DiskProviderFixtureBase<TSubject> : TestBase<TSubject> where TSubject : class, IDiskProvider
    {
        ...
        [Test]
        public void should_be_able_to_rename_open_hardlinks_with_fileshare_write()
        {
            DoHardLinkRename(FileShare.Write);
        }

        [Test]
        public void should_be_able_to_rename_open_hardlinks_with_fileshare_none()
        {
            DoHardLinkRename(FileShare.None);
        }
    }
}
