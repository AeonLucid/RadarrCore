...
using Radarr.Common.Interop;

namespace Radarr.Common.Disk
{
    public abstract class DiskProviderBase : IDiskProvider
    {
        private static readonly Logger Logger = RadarrLogger.GetLogger(typeof(DiskProviderBase));
        ...
        {
            Ensure.That(path, () => path).IsValidPath();
            return FileExists(path, OsInfoCore.PathStringComparison);
        }
